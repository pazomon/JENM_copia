...
class Planificador(Frame):
    ...
    def initUI(self):
        ...
        self.button_inplans = Button(self.frameOne, text="Planes de Estudios", command=self.inpe, width=21)
        self.button_inplans.grid(row=6, column=0)

    def AuxscrollFunction(self,event):
        self.canvas.configure(scrollregion=self.canvas.bbox("all"),width=1150,height=400)

    def inpe (self):
        self.button_inpe_mod = Button(self.frameOne, text="Modificar", width=50)
        self.button_inpe_mod.grid(row=6, column=1)

        self.button_inpe_load = Button(self.frameOne, text="Cargar archivo", command=self.select_inpe, width=50)
        self.button_inpe_load.grid(row=7, column=1)

    def select_inpe (self):
        file = filedialog.askopenfilename(initialdir="/",title="selecionar")
        curriculums.load_pe(file)
    ...
    def img_marks(self):
        ...
        self.anadirpiezas = Button(self.frameThree, text="Cargar notas", command=self.addmarks, width=10)
